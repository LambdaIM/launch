...
!!! example ""
    ...
    2. 下载安装包
    ```
    wget https://github.com/LambdaIM/launch/releases/download/Storage0.2.7.2/lambda-storage-0.2.7.2.tar.gz
    ```
    如下载缓慢可使用下面的链接：
    ```
    wget http://download.lambdastorage.com/lambda-storage/0.2.7.2/lambda-storage-0.2.7.2.tar.gz
    ```
    3. 解压安装包
    ```
    tar zxvf lambda-storage-0.2.7.2.tar.gz
    ```
    4. 进入解压后的目录
    ```
    cd lambda-storage-0.2.7.2
    ```
