...
主网节点升级可参考：[主网Validator 0.5.3 升级教程](Mainnet-Validator-Upgrade-Guide.md)  
...
```
解压`lambda_0.5.3_data.tar.gz`到`~/.lambda/data/`目录下（解压过程耗时较长，请耐心等待）
``` 
tar -zxvf lambda_0.5.3_data.tar.gz -C ~/.lambda/data/
```
启动节点
```
./lambda start --p2p.laddr tcp://0.0.0.0:26656 --rpc.laddr tcp://0.0.0.0:26657 --daemonize --log.file /tmp/lambda.log
