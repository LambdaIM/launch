...
使用矿工管理程序配合钱包即可完成挖矿所需要各项操作，非常便利
...
2 挖矿子账户有选择本地文件和选择本地钱包导出的挖矿子账户两种方式
  ...
  如果本机也安装了lamb钱包，钱包导出的挖矿子账户就会出现在下啦列表中
...
矿工节点配置分4个模块 分别是api_key,bd,kad,log,server
...
~/.lambda_miner/config/config.toml，参考如下说明进行配置
...
```
配置完成后可以点击启动按钮，启动挖矿程序，成功后状态会便车个绿色的☑️
...
~/.lambda_storage/config/config.config.toml 配置说明
